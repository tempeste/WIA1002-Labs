...
public class BST<T extends Comparable<T>>{
    ...
    public T findMin(BSTNode<T> a){
        if(a.getLeft() == null){
            return a.getData();
        }else{
            return (T) findMin(a.getLeft());
        }
    }
    
    public T findMax(){
        return (T) findMax(root);
    }
    
    public T findMax(BSTNode<T> a){
        if(a.getRight() == null){
            return a.getData();
        }else{
            return (T) findMax(a.getRight());
        }
    }
    ...
    public int findTotal(BSTNode a){
        if(a.getLeft() == null && a.getRight() == null){
            return (int) a.getData();
        }else if(a.getLeft() == null){
            return (int) a.getData() + findTotal(a.getRight());
        }else if(a.getRight() == null){
            return (int) a.getData() + findTotal(a.getLeft());
        }else{
            return (int) a.getData() + findTotal(a.getLeft()) + findTotal(a.getRight());
        }
    }
    
    public int findOccurrence(T a){
        return findOccurrence(root, a, 0);
    }
    
    private int findOccurrence(BSTNode<T> node, T a, int count ){
        if(node == null){
            return count;
        }else if(a.compareTo(node.getData()) < 0 ){
            return findOccurrence(node.getLeft(), a, count);
        }else if(a.compareTo(node.getData()) > 0 ){
            return findOccurrence(node.getRight(), a, count);
        }else{
            return count + 1 + findOccurrence(node.getRight(), a, count) 
                    + findOccurrence(node.getLeft(), a, count);
        }
    }
}
